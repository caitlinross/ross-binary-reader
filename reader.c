...
void event_read(FILE *file, FILE *output)
{
    ...
    if (g_combine)
        ...
        fprintf(output, "src_lp,dest_lp,recv_ts_vt,send_ts_vt,recv_ts_rt,duration,local_id,global_id,event_time,neuronvoltage,event_type\n");
...
void print_event_struct(FILE *output, event_line *line)
{
    ...
    fprintf(output, "%"PRIu64",%"PRIu64",%f,%f,%f,%f,%"PRIuFAST16",%"PRIu64",%f,%d,%"PRIuFAST8"\n", line->src_lp, line->dest_lp, line->recv_ts_vt, line->send_ts_vt, line->recv_ts_rt, line->duration, line->localID, line->globalID, line->eventTime, line->neuronVoltage, line->event_type);
}
